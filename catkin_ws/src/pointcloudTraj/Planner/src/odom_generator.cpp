...
bool rcv_cmd = false;
//Callback function that is called whenever a position command is received. It updates the _cmd variable with the received command.
void rcvPosCmdCallBack(const quadrotor_msgs::PositionCommand cmd)
{	
	rcv_cmd = true;
	_cmd    = cmd;
}
// Publishes odometry information based on the received position command or the initial position parameters. 
//The published odometry information is sent on the odometry topic.
void pubOdom()
{	
