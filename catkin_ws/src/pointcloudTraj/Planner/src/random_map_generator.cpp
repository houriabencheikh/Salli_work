...
// simulates a scenario where a quadrotor explores a randomly generated map of vertical obstacles and publishes 
//the global and local maps. The local map is updated based on the quadrotor's position,
// and only the points within the specified sensing range and field of view are included in the local map.
...
{    
   ...
   //(cloudMap) representing the entire map
...
//This callback function receives odometry information. It updates the current state _state with the position and velocity information from the odometry message.
