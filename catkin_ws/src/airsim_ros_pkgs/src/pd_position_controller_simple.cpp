...
void PIDPositionController::initialize_ros()
{
    ...
    //vfh_cmd_sub_ = nh_.subscribe("/vfh3d/cmd_pose", 10, &PIDPositionController::vfhCmdCallback, this);
...
/*
void PIDPositionController::vfhCmdCallback(const geometry_msgs::PoseStamped::ConstPtr& cmd_pose) {
    tf::Quaternion quat;
    tf::quaternionMsgToTF(cmd_pose->pose.orientation, quat);
    quat.normalize();

    // Convert back to ROS message type
    geometry_msgs::Quaternion normalized_orientation;
    tf::quaternionTFToMsg(quat, normalized_orientation);


    double yaw = tf::getYaw(normalized_orientation);
    ROS_INFO("Yaw from normalized quaternion: %f", yaw);

    // Update the target position based on VFH command
    target_position_.x = cmd_pose->pose.position.x;
    target_position_.y = cmd_pose->pose.position.y;
    target_position_.z = cmd_pose->pose.position.z;
    target_position_.yaw = tf::getYaw(normalized_orientation);

    // Set flag indicating a new target has been received
    has_goal_ = true;
    reached_goal_ = false;
}
*/